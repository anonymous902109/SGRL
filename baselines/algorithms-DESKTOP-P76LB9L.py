...
def neme_algorithm(clf, DIVERSITY_SIZE, dataset, results_path):
	...
	ga_df = pd.read_csv(os.path.join(results_path, 'SGEN/{}_{}.csv'.format(DIVERSITY_SIZE, dataset.outcome_name)))
...
def piece_algorithm(clf, DIVERSITY_SIZE, dataset, results_path):
	...
	cat_idxs = generate_cat_idxs()
	ga_df = pd.read_csv(os.path.join(results_path, 'SGEN/{}_{}.csv'.format(DIVERSITY_SIZE, dataset.outcome_name_name)))
	test_idxs = np.sort(np.array(ga_df.test_idx.value_counts().index.tolist()))
	piece_sfs = list()
	found_sf = list()
	idx_data = list()
...
def genetic_algorithm(clf, DIVERSITY_SIZE, POPULATION_SIZE, dataset, results_path, test_size):
	...
	sf_df_readable.to_csv(os.path.join(results_path, 'SGEN/{}_{}_readable.csv'.format(DIVERSITY_SIZE, dataset.outcome_name)), index=False)

	sf_df['test_idx'] = idx_data
	sf_df['fact_id'] = [fact_ids[i] for i in idx_data.squeeze()]
	sf_df['sf_found'] = success_data

	sf_df.to_csv(os.path.join(results_path, 'SGEN/{}_{}.csv'.format(DIVERSITY_SIZE, dataset.outcome_name)), index=False)
